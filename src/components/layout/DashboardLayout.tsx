...
const DashboardLayout: React.FC<{ title: string; items: NavItem[]; children: React.ReactNode }> = ({ title, items, children }) => {
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground transition-colors duration-500">
      ...
      <aside className="w-72 glass border-r border-foreground/5 shrink-0 hidden md:flex flex-col relative z-20 overflow-hidden">
        ...
        <div className="p-10 pb-12 relative z-10">
          <Link to="/" className="flex items-center gap-4 group">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mt-1 opacity-60">Enterprise HUB</p>
            </div>
          </Link>
        </div>
           ...
           <div className="p-6 rounded-[2.5rem] glass-card border border-foreground/5 shadow-premium">
              <div className="flex items-center gap-4 mb-6">
                 ...
                 <div className="flex-1 min-w-0">
                    <p className="text-xs font-display font-black text-foreground truncate italic uppercase tracking-tighter">{userProfile?.displayName || "Operative"}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 transition-all duration-700">
        ...
        <header className="md:hidden sticky top-0 z-50 glass border-b border-foreground/5 flex items-center h-20 px-6 gap-4">
          ...
          <button onClick={() => { logout(); navigate("/"); }} className="h-11 w-11 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center"><LogOut className="h-5 w-5" /></button>
        </header>

        {/* Mobile Sub-nav */}
        <nav className="md:hidden flex gap-3 overflow-x-auto scrollbar-hide px-6 py-4 glass border-b border-foreground/5 no-scrollbar">
          {items.map((item) => (
            <Link key={item.path} to={item.path}
        ...
        <main className="flex-1 p-8 md:p-12 overflow-auto scrollbar-hide bg-background/50">
