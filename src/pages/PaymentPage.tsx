...
const PaymentPage = () => {
  const { total, clearCart, currentItems: items, subtotal, deliveryFee, tax, discount, stopSharingCart } = useCart();
  const { user, userProfile } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const [selected, setSelected] = useState("upi");
  const isCodAllowed = (userProfile?.onlineOrderCount || 0) >= 3;
  ...
  if (!location.state?.addressId) {
    toast.error("Logistics Protocol Violation: Destination address missing.");
    navigate("/checkout");
    return null;
  }
  ...
  const executeOrder = async () => {
    ...
    try {
      ...
      let orderPincode = "";
      let orderFullAddress = location.state.addressFull;
      ...
      // 4. Clear cart and stop sharing
      await clearCart();
      stopSharingCart();

      // 5. Update User's online order count if payment was not COD
      if (selected !== "cash") {
        const newCount = (userProfile?.onlineOrderCount || 0) + 1;
        await setDoc(doc(db, "users", user.uid), {
          onlineOrderCount: newCount
        }, { merge: true });
      }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container max-w-lg pb-20 px-4 animate-in fade-in duration-700">
        <div className="flex items-center gap-4 pt-10 pb-6">
          <Link to="/checkout" className="h-10 w-10 rounded-2xl glass border border-foreground/5 flex items-center justify-center text-foreground hover:border-primary/20 transition-all">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="font-display font-black text-2xl text-foreground tracking-tight uppercase italic">Secure Payment</h1>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-4 mb-2">Selection Logic</p>
          {paymentMethods.map((pm) => {
            const isLocked = pm.id === "cash" && !isCodAllowed;
            return (
              <button
                key={pm.id}
                onClick={() => !isLocked && setSelected(pm.id)}
                disabled={isLocked}
                className={`w-full p-5 rounded-[2rem] border transition-all text-left group relative ${selected === pm.id
                    ? "border-primary bg-primary/5 shadow-lg shadow-primary/5"
                    : isLocked
                      ? "border-foreground/5 bg-foreground/5 opacity-60 cursor-not-allowed"
                      : "bg-card border-foreground/5 hover:border-foreground/10"
                  }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-all ${selected === pm.id ? 'bg-primary text-white' : 'bg-foreground/5 text-muted-foreground'
                    }`}>
                    <pm.icon className="h-6 w-6" />
                  </div>
                  <div className="flex-1">
                    <p className={`font-display font-bold text-sm ${selected === pm.id ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {pm.label}
                      {isLocked && <span className="ml-2 text-[8px] bg-rose-500/20 text-rose-500 px-2 py-0.5 rounded-md italic">LOCKED</span>}
                    </p>
                    <p className="text-xs text-muted-foreground font-medium">
                      {isLocked ? `Complete ${3 - (userProfile?.onlineOrderCount || 0)} more online orders to unlock` : pm.desc}
                    </p>
                  </div>
                  {!isLocked && (
                    <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center transition-all ${selected === pm.id ? 'border-primary' : 'border-foreground/20'}`}>
        ...
        {selected !== "cash" && (
          <div className="p-8 rounded-[2.5rem] bg-indigo-500/10 border border-indigo-500/20 mb-8 animate-in zoom-in duration-500 text-center relative overflow-hidden group">
            ...
            <h3 className="font-display font-black text-foreground italic uppercase tracking-tighter text-lg mb-2">Digital Escrow</h3>
        ...
        {selected === "cash" && (
          <div className="p-6 rounded-[2.5rem] glass-card border border-foreground/5 mb-8 animate-in slide-in-from-top-4 duration-500">
            <p className="text-sm text-muted-foreground font-medium leading-relaxed">Pay <span className="text-foreground font-black">₹{total}</span> in currency when our executive reaches your coordinate. Please keep exact change ready.</p>
          </div>
        )}

        {/* Security badge */}
        <div className="flex items-center justify-center gap-3 mb-8 p-3 rounded-2xl glass-card border border-foreground/5">
          <Shield className="h-4 w-4 text-emerald-500" />
          <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em]">Validated by AES-256 TLS Tunnel</span>
        </div>
